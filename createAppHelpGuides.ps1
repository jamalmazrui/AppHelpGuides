# createAppHelpGuides.ps1
# Creates the AppHelpGuides Git repository, connects it to GitHub, and pushes.
# Writes createAppHelpGuides.log beside itself.
#
...
param(
    [string] $sRemoteName = "AppHelpGuides",
    [switch] $bPrivate,
    [switch] $bPushOnly
)
...
$pathLog = Join-Path $pathRoot "createAppHelpGuides.log"
...
writeLog "createAppHelpGuides starting in $pathRoot"
...
# fails is worth more than explaining the rejection afterwards. Harvest
# corpora are the usual offenders here, which is why .gitignore excludes them.
$bOversize = $false
...
if ($result.Output) {
    $sMessage = if ($bPushOnly) { "Update" } else { "AppHelpGuides: screen reader friendly compendia of official product help" }
    writeLog "Committing"
...
if (-not $sOrigin) {
    ...
    gh repo create $sRemoteName $sVisibility --source=. --remote=origin --description "Screen reader friendly Markdown compendia of official help documentation for widely used apps and services"
} else {
...
writeLog "createAppHelpGuides finished"
